-- 1) Rooms RLS — staff can read rooms across the whole facility
DROP POLICY IF EXISTS "staff read own floor rooms" ON public.rooms;
DROP POLICY IF EXISTS "staff read facility rooms" ON public.rooms;

CREATE POLICY "staff read facility rooms" ON public.rooms
  FOR SELECT
  USING (
    floor_id IN (
      SELECT id FROM public.floors
      WHERE facility_id = (SELECT facility_id FROM public.profiles WHERE id = auth.uid())
    )
  );

-- 2) Update submit_round_scan: only the success branch's return message changes.
...
BEGIN
  ...
  IF v_open_slot IS NOT NULL THEN
    ...
    RETURN jsonb_build_object(
      ...
      'code','completed',
      'title','Checked in',
      'message','This QR code is for Room '||v_room.room_number||'. Thank you for checking.',
