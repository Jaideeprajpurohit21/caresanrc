...
BEGIN
  ...
  IF v_room.id IS NULL THEN
    RETURN jsonb_build_object('ok', false, 'code','unknown_qr',
      ...
      'message','This code or tag is not registered to any active room.',
      'dry_run', p_dry_run);
  END IF;
  ...
  IF v_open_slot IS NOT NULL THEN
    ...
    -- Next scheduled round for this room after the one just completed
    IF v_open_round < v_schedule.rounds_per_shift THEN
      v_next_slot := v_anchor + v_schedule.frequency * (v_open_round + 1);
    ELSE
    ...
    RETURN jsonb_build_object(
      ...
      'message','Room '||v_room.room_number||' checked in. Next scan is scheduled at '||
